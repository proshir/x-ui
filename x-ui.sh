...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/proshir/x-ui/my-dev/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/proshir/x-ui/raw/my-dev/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
